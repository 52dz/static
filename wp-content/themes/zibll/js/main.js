 ...
 * @LastEditTime: 2022-04-28 23:09:33
...
$.fn.minitouch = function (options) {
    ...
    _e.on('touchstart pointerdown MSPointerDown', touch_selector, function (e) {
            ...
            dragging = !0;
            //兼容swiper
            if ($(e.target).parentsUntil(touch_selector, '.swiper-container').length) {
                dragging = !1;
            }
        })
...
_win.bd.on('shown.bs.tab', 'a[data-ajax]', function (e) {
    $('.tab-nav-theme,.index-tab>ul').Tabcenter();
});
$('.tab-nav-theme,.index-tab>ul').Tabcenter();
            ...
            $fields.each(function () {
                ...
                $.each(controllers, function (index, depend_id) {
                    ...
                    $this.on('change', "[name='" + depend_id + "']", function (elem) {
                        ...
                        if (evalCondition(condition, value, val2)) {
                            //  $field.slideDown(300)
                            $field.show()
                        } else {
                            $field.hide()
        ...
        setTimeout(function () {
            if (lazyload_action == 'ias') {
                if (_this.attr('remote-box')) {
                    ...
                    var _a = _this.hasClass('ajax-next') ? _this : _this.find('.ajax-next');
...
console.log('\n' + ' %c ZibllTheme %c https://loowp.com ' + '\n', 'color: #fadfa3; background: #030307; padding:5px 0; font-size:12px;', 'background: #fadfa3; padding:5px 0; font-size:12px;');